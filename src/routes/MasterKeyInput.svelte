<script lang="ts">
	...
	let value: String;
	export let check_master = false;

	async function set_master_key() {
		check_master = false;
		const res = await fetch(`${PUBLIC_API_ENDPOINT}/key?${value}`, {
			credentials: 'include',
			mode: 'cors'
		});
		if (res.ok) document.cookie = 'is_master=true;max-age=31536000';
	}
</script>

<div class="mt-4 relative flex h-10 w-full min-w-[200px] max-w-[24rem]">
	<input
		bind:value
